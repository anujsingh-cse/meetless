...
async function main() {
  console.log('=== Claude Code Connector Demo ===\n')
  ...
  const team = await prisma.team.create({ data: { name: 'Demo Team', slug: `demo-team-${Date.now()}` } })
  const user = await prisma.user.create({ data: { clerkId: `clerk_demo_${Date.now()}`, email: `demo-${Date.now()}@example.com` } })
  const workspace = await prisma.workspace.create({ data: { teamId: team.id, name: 'Demo Workspace' } })
  const session = await prisma.session.create({ data: { workspaceId: workspace.id, userId: user.id, name: 'Demo Session' } })
  ...
  // Patch spawn for the duration of the demo
  ...
  // 5. Simulate Agent A editing a file
  console.log('\nAgent A editing src/app.ts ...')
  await pipeline.handleEvent({
    sessionId: session.id,
    agentId: 'claude-1',
    tool: 'edit_file',
    params: { path: 'src/app.ts', content: 'function main() { return "A" }' },
    result: { success: true },
    timestamp: Date.now(),
  ...
  // 6. Simulate Agent B editing the SAME file (conflict)
  console.log('\nAgent B editing src/app.ts ...')
  await pipeline.handleEvent({
    sessionId: session.id,
    agentId: 'claude-2',
    tool: 'edit_file',
